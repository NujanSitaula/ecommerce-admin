...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/admin/coupons${queryString ? `?${queryString}` : ""}`;
    ...
    return NextResponse.json(
      {
        message: error instanceof Error ? error.message : "Unable to fetch coupons",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        message: error instanceof Error ? error.message : "Unable to create coupon",
      },
      { status: 500 }
    );
  }
