...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/admin/countries${queryString ? `?${queryString}` : ""}`;
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Unable to fetch countries",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/api/admin/countries`;
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { message: errorData.message || "Failed to create country" },
        { status: response.status }
      );
    }
